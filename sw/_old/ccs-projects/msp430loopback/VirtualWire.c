...
/*#if defined(ARDUINO)	// MSP430 Port
 #if (ARDUINO < 100)
  #include "WProgram.h"
 #endif
...
static uint8_t vw_tx_buf[(VW_MAX_MESSAGE_LEN * 2) + VW_HEADER_LEN] 
     = {0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x2a, 0x38, 0x2c};
...
/*static uint8_t _timer_calc(uint16_t speed, uint16_t max_ticks, uint16_t *nticks)		//MSP430 Port
{
...
#else // defined(__MSP430G2452__) || defined(__MSP430G2553__) // LaunchPad specific	// MSP430 Port
void vw_Int_Handler()
{
    if (vw_rx_enabled && !vw_tx_enabled)
